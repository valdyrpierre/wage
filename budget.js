// TODO: replace with your actual Supabase project values
        ...
        async function init() {
            ...
            await refreshTotals();
            // if not currently clocked in, show today's total instead of the
            // default $0.00 placeholder (the live timer will take over and
            // update this itself if we ARE currently clocked in)
            if (!activeShift) {
                document.getElementById('live-pay').textContent = document.getElementById('total-day').textContent;
            }
        ...
        document.getElementById('clock-out').addEventListener('click', async () => {
            ...
            await refreshTotals();
            // show today's actual total (from the Day box we just refreshed) instead
            // of resetting to $0 — this keeps showing what was earned today even
            // after clocking out
            document.getElementById('live-pay').textContent = document.getElementById('total-day').textContent;
        });
